...
**Patch (post-deploy bug fixes):**
- Fixed a white-text bug on the Projects page list: `.tone-plum`/`.tone-navy` were defined as bare global classes for the Service cards, which leaked onto anything else reusing those class names. Now scoped to `.service-card.tone-plum` etc.
- Fixed the Projects page footer missing its "Let's connect" social links (`projects.js` never rendered them; `main.js` and `contact.js` did).
- Added inline positioning as a safety net directly on `<canvas id="pageParticles">` in all three pages, so it can never render at its raw pixel size (which was inflating page height dramatically) if the external stylesheet is slow to apply.
- Hid the hero's decorative blob shape below the mobile breakpoint (it had no reliable box to sit in once the layout stacks, causing it to render as a hard, unblurred shape overlapping the hero heading).
- Scaled the particle background down on narrow viewports (fewer, smaller, dimmer particles) so it reads as ambient texture rather than visual noise on small screens.
- Hardened the Projects page's list/detail grid against squeeze artifacts on unexpectedly narrow viewports.
